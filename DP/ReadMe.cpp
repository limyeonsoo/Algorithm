...
Dynamic Programming 
     * 2가지 속성을 만족해야 한다. 
     ...
     1. Overlapping Subproblem
        => 겹치는 부분 문제.
        ex )  피보나치 수열 :   F (n-2) + F (n-1) = F (n)

     2. Optimal Substructure
        => 문제의 정답을 작은 문제의 정답에서 구할 수 있을 때 
        ex ) 대전 -> 부산  가는 길 중 빠른 길이  대전 -> 대구 -> 부산  이라면  부산을 빨리 가려면 무조건 대구를 거쳐야 한다.
        즉, 이 전의 답을 이용해야 하는 것.

        10번 째 피보나치 수를 계산 하기 위해 구한 4번 째 피보나치 수,
        15번 째 피보나치 수를 계산 하기 위해 구한 4번 째 피보나치 수 

        중 4번 째 피보나치 수는 항상 같다. 
        이 정답을 어딘가에 저장 해놓아야 한다. =>  Memorization
     
     * 2가지 방법 
        ** 구현의 차이일 뿐,
     1. Top - down      // 큰 문제를 작게 만들어가면서 푼다.
        * 시간복잡도를 계산하기 힘들 수 있다. 
          ( memo[] 칸의 수 / N ) * 1칸을 채우는데 필요한 복잡도 


     2. Bottom - up     // 작은 문제부터 차례대로 푼다.
        작은 문제를 모두 풀었다면 큰 문제는 무조건 풀 수 있다.


    * 해결 방법 
      d/dp [ ] 배열에 무엇을 메모 해야할지 생각한다.
...
*/
//https://web.archive.org/web/20150427205243/https://www.acmicpc.net/wiki/%EC%95%8C%EA%B3%A0%EB%A6%AC%EC%A6%98/%EB%8F%99%EC%A0%81_%EA%B3%84%ED%9A%8D%EB%B2%95_-_dynamic_programming
/*
...
(0) https://www.acmicpc.net/problem/2579 0.5
(1) https://www.acmicpc.net/problem/5060 1
(2) https://www.acmicpc.net/problem/2533 1.5
(3) http://codeforces.com/contest/235/problem/B 2
(4) http://codeforces.com/contest/464/problem/C 2.5
(5) http://www.acmicpc.net/problem/8902 2.5
(6) https://www.acmicpc.net/problem/9338 3
